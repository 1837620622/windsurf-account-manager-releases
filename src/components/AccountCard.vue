<template>
  ...
  >
    ...
    <div class="card-body">
      ...
      <!-- 弹性计费（Usage Allowance）配额显示 -->
      <div class="elastic-quota-section" v-if="hasElasticQuota">
        <div class="quota-header">
          <div class="quota-header-left">
            <el-tag v-if="account.plan_name" :class="['plan-tag', `plan-${account.plan_name?.toLowerCase()}`]" size="small">
              <el-icon>
...
const dailyResetText = computed(() => {
  if (!props.account.dailyQuotaResetTimestamp) return '';
  const resetTime = dayjs.unix(props.account.dailyQuotaResetTimestamp);
  const now = dayjs();
  const diffHours = resetTime.diff(now, 'hour');
  const diffMinutes = resetTime.diff(now, 'minute') % 60;
  if (diffHours <= 0 && diffMinutes <= 0) return '即将重置';
  if (diffHours < 1) return `${diffMinutes}分钟后重置`;
  return `${diffHours}小时${diffMinutes > 0 ? diffMinutes + '分钟' : ''}后重置`;
});

// 每周配额重置时间文本
const weeklyResetText = computed(() => {
  if (!props.account.weeklyQuotaResetTimestamp) return '';
  const resetTime = dayjs.unix(props.account.weeklyQuotaResetTimestamp);
  const now = dayjs();
  const diffDays = resetTime.diff(now, 'day');
  const diffHours = resetTime.diff(now, 'hour') % 24;
  if (diffDays <= 0 && diffHours <= 0) return '即将重置';
  if (diffDays < 1) return `${diffHours}小时后重置`;
...
/* 暗色主题下的弹性计费区块 */
:root.dark .elastic-quota-section {
  background: linear-gradient(135deg, #2a2a2a 0%, #252525 100%);
  border: 1px solid rgba(255, 255, 255, 0.08);
}

:root.dark .elastic-quota-section:hover {
  background: linear-gradient(135deg, #2f2f2f 0%, #2a2a2a 100%);
}

:root.dark .elastic-quota-label {
  color: #9ca3af;
}

:root.dark .elastic-quota-value {
  color: #e5e7eb;
}

:root.dark .elastic-quota-reset {
  color: #6b7280;
}
