...
- **新增欢迎门户页**：启动软件后需输入访问口令 **`cknb`** 进入主界面
- **修复新账号导入**：解决部分新注册账号因 Firebase Referer 拦截导致无法导入的问题
- **安全加固**：全面拦截 DevTools 快捷键（Windows / macOS）、CSP 策略限制脚本注入
- **macOS 兼容**：修复 Cmd+Option+I 等 macOS 专属调试快捷键未拦截的问题

## 使用说明

> 启动软件后，会显示欢迎门户页面。请在输入框中输入访问口令 **`cknb`**，然后点击「进入」按钮即可进入主界面。
