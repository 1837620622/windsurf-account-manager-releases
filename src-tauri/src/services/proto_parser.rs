...
impl ProtobufParser {
    ...
    pub fn parse_get_plan_status_response(response_body: &[u8]) -> Result<Value, String> {
        ...
        if let Some(plan_status) = parsed.get("subMesssage_1") {
            ...
            // ====== 弹性计费（Usage Allowance）新字段 ======
            // field 12: on_trial (是否试用期)
            if let Some(v) = plan_status.get("int_12").and_then(|v| v.as_i64()) {
                result["on_trial"] = json!(v == 1);
            }
            // field 14: daily_quota_remaining_percent (每日配额剩余百分比)
            if let Some(v) = plan_status.get("int_14").and_then(|v| v.as_i64()) {
                result["daily_quota_remaining_percent"] = json!(v);
            }
            // field 15: weekly_quota_remaining_percent (每周配额剩余百分比)
            if let Some(v) = plan_status.get("int_15").and_then(|v| v.as_i64()) {
                result["weekly_quota_remaining_percent"] = json!(v);
            }
            // field 17: daily_quota_reset_timestamp (每日配额重置时间戳)
            if let Some(v) = plan_status.get("int_17").and_then(|v| v.as_i64()) {
                result["daily_quota_reset_timestamp"] = json!(v);
                // 转换为可读时间
                if let Some(dt) = chrono::DateTime::from_timestamp(v, 0) {
                    result["daily_quota_reset_time"] = json!(dt.format("%Y-%m-%d %H:%M:%S UTC").to_string());
                }
            }
            // field 18: weekly_quota_reset_timestamp (每周配额重置时间戳)
            if let Some(v) = plan_status.get("int_18").and_then(|v| v.as_i64()) {
                result["weekly_quota_reset_timestamp"] = json!(v);
                // 转换为可读时间
                if let Some(dt) = chrono::DateTime::from_timestamp(v, 0) {
                    result["weekly_quota_reset_time"] = json!(dt.format("%Y-%m-%d %H:%M:%S UTC").to_string());
                }
            }
        }
